<template>
  <div class="onboarding" @touchstart="onTouchStart" @touchend="onTouchEnd">
    ...
    <!-- Fallback gradient when video fails or loads -->
...
const videoBg        = ref(true)

// ✅ FIXED: Use BASE_URL so videos resolve on both web dev server and Capacitor Android build
...
const slides = [
  ...
  },
  {
    ...
    video: `${BASE}/videos/onboard_connect.mp4`,
  },
]
...
.bg-video {
  ...
  object-fit: cover;
  z-index: 0;
  /* Slightly dimmed so text is always readable */
  opacity: 0.75;
}
.video-overlay {
  position: absolute; inset: 0;
...
.slides-container {
  ...
  text-align: center; padding: 0 2rem; margin-bottom: 2rem;
...
.slide-headline {
  ...
  font-size: 2.2rem; font-weight: 700; color: #f0f4ff;
  margin: 0 0 1rem; white-space: pre-line; line-height: 1.2;
}
.slide-body {
  font-family: 'DM Sans', sans-serif;
  font-size: 0.95rem; color: rgba(240,244,255,0.7); line-height: 1.6; margin: 0;
}
...
.skip-btn {
  ...
  color: rgba(240,244,255,0.5); font-family: 'DM Sans', sans-serif;
  font-size: 0.9rem; padding: 14px 20px; border-radius: 14px; cursor: pointer;
}
.next-btn, .get-started-btn {
  flex: 1;
  background: linear-gradient(135deg, #C9A84C, #a8893d); border: none;
  color: #04071a; font-family: 'DM Sans', sans-serif;
  font-size: 1rem; font-weight: 700; padding: 16px; border-radius: 14px; cursor: pointer;
}
