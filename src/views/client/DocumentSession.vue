    ...
    <div class="doc-content">
        ...
        <div class="step-content">
          ...
          <div v-if="!briefSubmitted">
            <textarea v-model="brief" rows="4" class="brief-input"
              :placeholder="docType === 'Review' ? 'E.g. Please review this NDA. Check for unfair clauses, missing standard protections, and any red flags for an Indian company...' : 'E.g. Draft an NDA for a 2-year software partnership between two Indian companies, including IP ownership clauses, confidentiality, and termination conditions...'">
            </textarea>
            ...
            <button class="submit-btn" :disabled="!brief.trim() || (docType === 'Review' && !uploadedFile) || submittingBrief" @click="submitBrief">
      ...
      <div class="step-card" :class="{ active: briefSubmitted && !docReceived, done: docReceived }">
        ...
        <div class="step-content">
          ...
          <p class="step-desc">The lawyer is working on your {{ docType === 'Review' ? 'document review' : 'document' }}. Usually takes 15–60 minutes.</p>
          ...
          <!-- Simulate completion button (for testing) -->
      ...
      <div class="step-card" :class="{ active: docReceived && !paid, done: paid }">
        ...
        <div class="step-content">
          ...
          <div v-if="docReceived && !paid" class="payment-step">
            ...
            <button class="pay-btn" :disabled="paying || walletStore.balance < completedPages * pricePerPage" @click="payAndDownload">
...
const docType     = ref(route.query.type || 'Drafting') // 'Review' or 'Drafting'
...
// Simulate lawyer completing the work (for testing)
...
function downloadDoc() {
  // Fake download - create a simple PDF-like blob
  const content = `Vakilr Document\n\nPrepared for your legal matter.\n\nThis is a simulated document for testing.\n\nDocument: ${docName.value}\nPages: ${completedPages.value}\n\n[Document content would appear here in production]`
...
.file-upload-area:active { border-color: rgba(201,168,76,0.4); background: rgba(201,168,76,0.04); }
...
.step-btn { width: 32px; height: 32px; border-radius: 50%; background: rgba(201,168,76,0.1); border: 1px solid rgba(201,168,76,0.3); color: #C9A84C; font-size: 1.2rem; cursor: pointer; display: flex; align-items: center; justify-content: center; transition: all 0.15s; }
.step-btn:active { background: rgba(201,168,76,0.25); }
...
.paid-badge { background: rgba(34,197,94,0.1); border: 1px solid rgba(34,197,94,0.3); color: #4ade80; font-family: 'DM Sans', sans-serif; font-